<template>
  <div id="yanzi-player-box">
    ...
    <div id="yanzi-player" :class="{ shoyanziist: isShoyanziist, showMusic: currShowMusic }" class="yanzi-white" :style="'z-index:' + zIndex">
      ...
      <div class="listAndLrc">
        ...
        <div v-show="isShoyanziistAndLrc" class="yanzi-playerList">
          ...
          <div class="listTools">
            <div class="searchBox">
              <input @change="startSearchMusic" id="searchMusic" type="text" v-model="searchMusic" placeholder="搜索歌名、歌手" />
              <span class="iconfont" @click="startSearchMusic">&#xe65d;</span>
            </div>

            <div @click="choiceMusicFile" class="yanzi-btn">上传歌曲</div>
            <input id="uploadMusicFile" ref="uploadMusicFile" accept=".mp3" type="file" style="display:none;" @change="selectMusicFile" />
          </div>
          <div class="musicListBox">
            <div @click="selectedListMusic(index)" class="musicItem" v-for="(item, index) in filterList" :key="index">
              <div class="musicName" :class="{ activeColor: musicIndex == index }" v-html="item.author + ' - ' + item.name"></div>
            </div>
          </div>
        </div>
        ...
        <div v-if="!isShoyanziistAndLrc" class="yanzi-lrc">
          <div class="tispNull" v-show="!filterList[musicIndex].lrc">
            ...
            <input id="lrcFileInput" type="file" ref="filElem" accept=".lrc" style="display:none;" @change="selectLrcFile" />
          </div>
          <div style="height:170px;" v-show="filterList[musicIndex].lrc"></div>
          <div class="lrcItems" :class="{ lrcActiveStyle: lrcColorIndex == index }" :lrc-active="currentTimeNumber == index ? 'Y' : ''" v-for="(item, index) in lrcArray" :key="index">
            {{ item }}
          </div>
      ...
      <div class="yanziAudio-main">
        ...
        <div class="mainHead">
          <!-- 歌手头像 -->
          <div @click="selectMusicIcon" class="yanziAudio-icon" :class="{ pausedLoop: !isPlay }">
            <!-- <img src="../assets/image/xz.jpg" /> -->
            <img v-if="filterList && filterList.length && filterList[musicIndex].image" :src="filterList[musicIndex].image" />
            <img v-else src="../assets/music.png" />
          </div>
        </div>

        <input type="file" accept=".gif,.png,.jpg,.jpeg" ref="musicIcon" @change="uploadMusicIcon" style="display:none;">
        ...
        <div class="tools">
          <span class="mainButton">
            ...
            <span @click="playAndPause" v-if="!isPlay" class="iconfont" style="font-size:30px !important;">&#xe60a;</span>
            <span @click="playAndPause" v-if="isPlay" class="iconfont" style="font-size:30px !important;color:#1989fa;">&#xe60b;</span>
          ...
          <span class="btn2Block">
            <!-- 随机播放 -->
            <span @click="openAndCloseRandom" v-show="isRandom" class="iconfont" style="font-size:16px !important;">&#xe6d8;</span>
            ...
            <!-- 全部循环 -->
            <span @click="isSingleLoop = !isSingleLoop" v-show="!isSingleLoop" class="iconfont" style="font-size:16px !important;vertical-align: text-top;">&#xe61f;</span>

            <span class="iconfont volumeIcon">&#xe606;
              <div class="volumeBlock">
                <!-- 音量 -->
                <input @change="changeVolume" id="volume" type="range" orient="vertical" v-model="volumeValue" /><span id="volumeValueNumber" style="">{{ volumeValue }}</span>
              </div>
            </span>
          </span>
        </div>
        ...
        <div class="tools2">
          ...
          <div class="listAndClose">
            <span class="speed">
              ...
              <input @change="setCurrentTime" type="range" :max="allTimeNumber" min="0" step="1" v-model="currentTimeNumber" />
              <span>{{ allTime }}</span>
            </span>
